...
class SqlConnector(
    private val url: String = "jdbc:postgresql://localhost:5432/recipedb",
    private val user: String = "recipe",
    private val password: String = "recipe-pass",
    private val schema: String = "recipe",
    ...
    fun connect(vararg tables: Table): Database {
        ...
        transaction(connect) {
            if (System.getenv("recipe.sql_drop_db")?.toBoolean() == true) {
                SchemaUtils.drop(*tables, inBatch = true)
                SchemaUtils.create(*tables, inBatch = true)
            } else if (System.getenv("recipe.sql_fast_migration").toBoolean()) {
