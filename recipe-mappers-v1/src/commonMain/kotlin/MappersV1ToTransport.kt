...
private fun RecipeStepBase.toTransport(): RecipeApiStepBase = RecipeApiStepBase(
    hold = hold.toTransportRecipe(),
    description = description
)

private fun RecipeStepWithImage.toTransport(): RecipeApiStepWithImage = RecipeApiStepWithImage(
    hold = hold.toTransportRecipe(),
    description = description,
    image = image
)

private fun IRecipeStep.toTransport() : IRecipeApiStep = when(this) {
    is RecipeStepBase -> toTransport()
    is RecipeStepWithImage -> toTransport()
}

private fun List<IRecipeStep>.toTransport() : List<IRecipeApiStep>? = this
    .map { it.toTransport() }
    .toList()
    .takeIf { it.isNotEmpty() }
...
private fun RecipeContext.toTransportCreate() : RecipeApiRecipeCreateResponse = RecipeApiRecipeCreateResponse(
    ...
    result = if (state == RecipeState.RUNNING) RecipeApiResponseResult.SUCCESS else RecipeApiResponseResult.ERROR,
